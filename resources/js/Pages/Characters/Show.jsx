...
export default function CharacterShow({ character, title }) {
  if (!character) {
    return (
      <AppLayout>
  ...
  const nicknames =
    (character.nicknames?.length ?? 0) > 0 ? character.nicknames.join(', ') : null;

  const voices = [];
  if ((character.voices?.length ?? 0) > 0) {
    character.voices.forEach((voice) => {
      const person = voice?.person?.name ?? '';
      const language = voice?.language ?? '';
      if (person) voices.push(`${person} (${language})\n`);
    });
  }
  ...
  return (
    <AppLayout>
      <main className="pt-4 flex justify-center">
        <div className="max-w-400 px-8">
          <h1 className="text-4xl font-bold mb-4">{character.name ?? title ?? 'Character'}</h1>
